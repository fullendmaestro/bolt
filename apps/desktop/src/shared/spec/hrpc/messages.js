...
const encoding6 = {
  ...
  decode(state) {
    ...
    return {
      ...
      channelKey: (version >= 2 && (flags & 16) !== 0) ? c.string.decode(state) : null
    }
  }
}
...
const encoding19_0 = c.array(c.string)
...
const encoding19 = {
  preencode(state, m) {
    ...
    if (version >= 2 && m.ownedChannels) encoding19_0.preencode(state, m.ownedChannels)
  },
  encode(state, m) {
    const flags = (version >= 2 && m.ownedChannels) ? 1 : 0

    c.uint.encode(state, flags)

    if (version >= 2 && m.ownedChannels) encoding19_0.encode(state, m.ownedChannels)
  },
  decode(state) {
    const flags = c.uint.decode(state)

    return {
      ownedChannels: (version >= 2 && (flags & 1) !== 0) ? encoding19_0.decode(state) : null
    }
  }
}
